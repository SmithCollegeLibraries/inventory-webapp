...
export default class Header extends Component {
    ...
    logOut = (e) => {
      ...
      window.location.href = 'https://libtools.smith.edu/sis-webapp'
    }

    render() {
      const storage = JSON.parse(sessionStorage.getItem('account'))
      const { account } = storage || ''
      const { first_name, type } = account || ''
        return (
          <div>
            <Navbar color="dark" dark expand="md">
              <NavbarBrand href="#!">SIS (Version 0.1.8)</NavbarBrand>
              ...
              <Collapse isOpen={this.state.isOpen} navbar>
                <Nav className="ml-auto" navbar>
                  ...
                  <NavItem>
                        <NavLink className="nav-link" activeStyle={{ color: '#007BFF' }} to="/ill">ILL</NavLink>  
                  </NavItem>   
                  <NavItem>
